...
export default async function AccountSitePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const listingResult = await supabase.from("website_listings").select("id,owner_id,name,url,normalized_domain,short_description,contact_email,status,moderation_status,removal_reason,removed_at,rejection_reason,submitted_at,approved_at,category_id,category_request_id").eq("id", id).maybeSingle();
