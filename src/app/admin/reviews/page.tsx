...
const errorMessages: Record<string, string> = {
  ...
  category_details: "Check the category name, slug, icon and sort order.",
...
function categorySlug(value: string) {
  return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 100) || "category";
}
...
export default async function AdminReviewsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ data: listings, error: listingError }, { data: categories }, { data: revisions }] = await Promise.all([
    ...
    supabase.from("categories").select("id,name,slug,icon_key,sort_order").eq("is_active", true).order("sort_order").order("name"),
  ...
  const [{ data: requests }, { data: subscriptions }, { data: profiles }] = await Promise.all([
    ...
    ownerIds.length ? supabase.from("billing_subscriptions").select("owner_id,status,current_period_end,trial_entitlement").in("owner_id", ownerIds) : Promise.resolve({ data: [] }),
  ...
  return (
    <main className="account-shell" id="main-content">
      ...
      {!listings?.length ? <section className="account-empty"><h2>Queue clear</h2><p>There are no new-category listings waiting for review.</p></section> : <div className="moderation-list">{listings.map((listing) => {
        ...
        const profile = listing.owner_id ? profilesById.get(listing.owner_id) : null;
        const requestedName = request?.requested_name ?? "";
        const requestedKey = categoryKey(requestedName);
        const likelyMatches = (categories ?? []).filter((category) => {
          const existingKey = categoryKey(category.name);
          return requestedKey && (existingKey === requestedKey || existingKey.includes(requestedKey) || requestedKey.includes(existingKey));
        });
        ...
        return <article className="form-card moderation-card" key={listing.id}>
          <div className="moderation-card-heading"><div><span className="eyebrow">Requested category</span><h2>{listing.name}</h2><a href={listing.url} target="_blank" rel="noopener noreferrer">{listing.url}</a></div><time>{new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(listing.submitted_at))}</time></div>
          <p>{listing.short_description}</p>
          <dl className="admin-review-details"><div><dt>Account reference</dt><dd>{profile?.display_name ? `${profile.display_name} · ` : ""}{listing.owner_id}</dd></div><div><dt>Administrative contact</dt><dd>{listing.contact_email ?? "Not provided"}</dd></div><div><dt>Entitlement</dt><dd>{subscription ? `${subscription.status}${subscription.trial_entitlement ? " (trial entitlement)" : ""}${subscription.current_period_end ? ` · through ${new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(subscription.current_period_end))}` : ""}` : "No subscription record"}</dd></div><div><dt>Listing status</dt><dd>{listing.status}</dd></div></dl>
          ...
          <div className="admin-review-actions">
            <details className="moderation-form category-create-form"><summary>Create Category &amp; Approve</summary><form action={moderateListingAction}><input type="hidden" name="listingId" value={listing.id} /><input type="hidden" name="intent" value="approve_new_category" />{likelyMatches.length > 0 && <p className="form-alert form-alert-error"><strong>Possible duplicate:</strong> {likelyMatches.map((category) => category.name).join(", ")}. Assign an existing category if it fits.</p>}<label>Category name<input name="categoryName" required minLength={2} maxLength={80} defaultValue={requestedName} /></label><label>Slug<input name="categorySlug" required minLength={1} maxLength={100} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" defaultValue={categorySlug(requestedName)} /></label><label>Icon key<select name="categoryIconKey" defaultValue="folder"><option value="folder">Folder</option><option value="briefcase">Briefcase</option><option value="monitor">Monitor</option><option value="book">Book</option><option value="spark">Spark</option><option value="heart">Heart</option><option value="compass">Compass</option><option value="leaf">Leaf</option><option value="news">News</option><option value="bag">Bag</option><option value="people">People</option><option value="activity">Activity</option><option value="map">Map</option></select></label><label>Sort order<input name="categorySortOrder" type="number" min={0} max={1000000} step={1} defaultValue={nextSortOrder} required /></label><button className="button button-accent">Confirm, Create &amp; Publish</button></form></details>
            <form action={moderateListingAction} className="moderation-form"><input type="hidden" name="listingId" value={listing.id} /><label>Use existing category<select name="categoryId" required defaultValue=""><option value="" disabled>Choose category</option>{(categories ?? []).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><button className="button button-secondary" name="intent" value="assign_existing">Assign to Existing Category</button></form>
